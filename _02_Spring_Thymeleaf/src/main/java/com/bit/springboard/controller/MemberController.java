...
@RequiredArgsConstructor // 생성자 생성하는 어노테이션
...
//        mav.addObject("name","회원가입");
    ...
    @PostMapping("/login")
    public ModelAndView login(MemberDto memberDto, HttpSession session, HttpServletResponse response /* 리다이렉트 시키기 위한 매개변수(로그인 후 url이 /로 바뀜 */) {
        ModelAndView mav = new ModelAndView();
    ...
    public void logout(HttpSession session, HttpServletResponse response) {
        try {
            session.invalidate(); // session.invalidate() : 세션 만료
